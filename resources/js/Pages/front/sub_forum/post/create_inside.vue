...
export default{
	components:{
	 ...
	 this.form.description=this.editor.getHTML();
			   ...
			   router.post(route('post_inside.store',{id : this.id} ), this.form, {
    ...
    >
      <div class="text-center p-4">
        ...
        <p class="text-gray-500 font-medium">{{ __("add_image") }}</p>
	      ...
	      <div v-for="(field, index) in postFormFields" :key="index" class="p-4 bg-white rounded-lg shadow-sm form-field">
      <label class="block text-sm font-medium text-gray-700 mb-1" :for="field.field_key">{{ field.field_label }}</label>
      ...
      <select 
        ...
        :required="field.required || false"
      >
        <option v-for="option in field.field_options" :value="option.option_text">
          {{ option.option_text }}
        </option>
      </select>
