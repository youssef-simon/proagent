...
<tr  v-for="area in areas.data">
	...
	<td>
		...
		<Link  class="mr-1 mb-1 px-4 py-3 text-sm leading-4 border rounded hover:bg-white focus:border-primary focus:text-primary"
                    ...
                    :href="'/area/edit/'+area.id"
                    v-html="'edit'"
                />
				
			 <Link class="btn-primary btn" @click="destroy( area.id)" >
                                                Delete
                                            </Link> 
				</td>
</tr>
	  ...
	  <div v-if="areas.links.length > 3">
        <div class="flex flex-wrap mt-8">
            <template v-for="(link, key) in  areas.links" :key="key">
                <div
                    v-if="link.url === null"
        ...
        Link,
		  ...
		  destroy(id) {
				if (confirm("Are you sure you want to Delete")) {
					...
					router.delete(route("area.delete", id));
				}
			},
/* 	 submit( ) {
